...
def download_manga(url):
    ...
    try:
        ...
        # Create a download folder with mangaID_mangaNAME format
        folder_name = f"{manga_id}_{name if name else pretty_name}"
...
def convert_to_pdf(manga_folder):
    """
    Convert all images in a manga folder to a single PDF file
    
    :param manga_folder: Path to the folder containing manga images
    :return: Path to the created PDF file, or None if conversion failed
    """
    try:
        # Get all image files in the folder
        image_files = []
        for file in sorted(os.listdir(manga_folder)):
            if file.lower().endswith(('.jpg', '.jpeg', '.png', '.webp')):
                image_files.append(os.path.join(manga_folder, file))
        
        if not image_files:
            print(f"No image files found in {manga_folder}")
            return None
        
        # Sort files by name to maintain page order
        image_files.sort()
        
        # Get the folder name for the PDF
        folder_name = os.path.basename(manga_folder)
        # Create PDF filename inside the manga folder
        pdf_path = os.path.join(manga_folder, f"{folder_name}.pdf")
    ...
    for url in urls:
        ...
        try:
            ...
            if downloaded_path and not failed_pages:
                ...
                # Convert to PDF after successful download
                pdf_path = convert_to_pdf(downloaded_path)
                if pdf_path:
                    print(f"Created PDF for manga: {pdf_path}")
            else:
